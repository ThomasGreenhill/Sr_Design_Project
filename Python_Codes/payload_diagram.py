...
Range = [0, R_B*km_to_miles*80/120, R_C*km_to_miles*80/120]
Wp = (MTOW-OEW) * N_to_lbs
Woe = OEW * N_to_lbs
E_reserve = E_1*2.77778e-7
E_b = E_0*2.77778e-7
...
#### Notations for Weight vs. Range part
plt.annotate('', xy=(50*km_to_miles*80/120,0), xytext=(50*km_to_miles*80/120,Woe), arrowprops=dict(arrowstyle="<->",
              connectionstyle="arc3", color=color1, lw=2))
plt.text(55*km_to_miles*80/120, 7600*N_to_lbs, "Operating\nEmpty\nWeight")

plt.annotate('', xy=(50*km_to_miles*80/120,Woe), xytext=(50*km_to_miles*80/120,Woe+Wp), arrowprops=dict(arrowstyle="<->",
              connectionstyle="arc3", color=color1, lw=2))
plt.text(55*km_to_miles*80/120, 11700*N_to_lbs, "Max Payload")
...
#### Notations for Battery Charge vs. Range part
plt.annotate('', xy=(230*km_to_miles*80/120,0), xytext=(230*km_to_miles*80/120,E_reserve), arrowprops=dict(arrowstyle="<->",
              connectionstyle="arc3", color=color2, lw=2))
plt.text(173*km_to_miles*80/120, 5, "Fuel Reserves")

plt.annotate('', xy=(240*km_to_miles*80/120,0), xytext=(240*km_to_miles*80/120,E_b+E_reserve), arrowprops=dict(arrowstyle="<->",
              connectionstyle="arc3", color=color2, lw=2))
plt.text(217*km_to_miles*80/120, 98, "Full\nFuel\nTank")

#### Final Step 
plt.title('Impact of Payload on Range - 40 kt Headwind') 
plt.savefig('%s/Payload_Diagram_Imperial_40ktHeadwind.png'%(savedir))
plt.show()