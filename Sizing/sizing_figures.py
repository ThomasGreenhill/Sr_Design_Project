...
def sizing_figures(Sizing, S_wing_LO, S_wing_HI, S_disk_LO, S_disk_HI, num):
    ...
    wing_loading_1 = numpy.zeros((num,))      # kg/m^2
    P_list = [0] * num
    ...
    S_disk_list = numpy.linspace(S_disk_HI, S_disk_LO, num)
    TOGW_S_disk = [0] * num
    ...
    # ax1.xaxis.grid()
    # ax1.yaxis.grid()
    plt.ylim((12500,20500))
    ax1.plot(S_disk_list, TOGW_S_disk, 'b-', label="Disk area")
    ax1.set_xlabel("Disk area (m$^2$)")
    ax1.scatter(20,13000,1000,color='k',marker='+')
    ax1.plot((5,20),(13000,13000),'k--')
    ax2.plot(disk_loading_list_SI, TOGW_S_disk, 'r-', label="Disk loading")
    ax2.set_xlabel("Disk loading (N/m$^2$)")
    ax2.scatter(650,13000,1000,color='k',marker='+',label="Initial Sizing Choice")
    ax2.plot((650,650),(20500,13000),'k--')
